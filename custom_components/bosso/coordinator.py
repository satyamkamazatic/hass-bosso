...
#         # either at the device level or inside one of its zones. Saves an
#         # API call for presets that don't need it (full-state presets).
...
class BossoCoordinator(DataUpdateCoordinator[dict[int, dict[str, Any]]]):
    ...
    async def async_load_predefined_presets(self) -> None:
        ...
        This one is safe to cache for the lifetime of the integration —
        predefined presets are static, unlike user-defined ones.
        Failures are logged and non-fatal — the dropdown just appears empty.
        """
        try:
            predefined = await self.api.async_list_presets(predefined=True)
            for preset in predefined:
    ...
    async def async_refresh_user_presets(self) -> None:
        ...
        Unlike predefined presets, user presets can be created, renamed,
        or deleted at any time from other clients (the Bosso app, etc.),
        so this is called on every coordinator poll cycle rather than
        once at setup. The cache is cleared and rebuilt each time so
        renamed/deleted presets don't linger as stale dropdown options.
 
        Failures are logged and non-fatal — on a transient failure we
        simply keep showing whatever was cached from the last successful
        refresh, rather than blanking the dropdown.
        """
        try:
            user = await self.api.async_list_presets(predefined=False)
        except BossoApiError as err:
    ...
    async def _async_update_data(self) -> dict[int, dict[str, Any]]:
        ...
        Also refreshes the user-defined preset catalog on every cycle
        (see async_refresh_user_presets for why — unlike predefined
        presets, these can change at any time from other clients).
        """
        try:
            devices = await self.api.async_list_devices()
        except BossoAuthError as err:
